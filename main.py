...
# from app.services.unified_ingestion_service import ingest_findings
...
#     semgrep = parse_semgrep_report("reports/semgrep.json")
#     gitleaks = parse_gitleaks_report("reports/gitleaks.json")
#     trivy = parse_trivy_report("reports/trivy.json")

#     all_findings = semgrep + gitleaks + trivy

#     ingest_findings(all_findings, db)

#     return {
#         "message": "Unified ingestion complete",
...
# -------------------------
@app.post("/findings")
def create_finding(finding: Finding, db: Session = Depends(get_db)):
    ...
    return {"message": "created", "id": new_finding.id}
...
def ingest_gitleaks(db: Session = Depends(get_db)):
    ...
    findings = parse_gitleaks_report("reports/gitleaks.json")
    ingest_findings(findings, db)
...
def ingest_trivy(db: Session = Depends(get_db)):
    ...
    findings = parse_trivy_report("reports/trivy.json")
    ingest_trivy_findings(findings, db)
...
def ingest_all(db: Session = Depends(get_db)):
    ...
    all_findings = (
        parse_semgrep_report("reports/semgrep.json") +
        parse_gitleaks_report("reports/gitleaks.json") +
        parse_trivy_report("reports/trivy.json")
    )
...
@app.post("/ingest/ci")
def ingest_ci(payload: dict, db: Session = Depends(get_db)):

    findings = payload.get("findings", [])

    if not findings:
        return {"message": "no findings received", "count": 0}

    inserted = ingest_findings(findings, db)

    return {
        "message": "CI ingestion complete",
        "count": inserted
    }